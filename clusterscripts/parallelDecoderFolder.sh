...
time mpiexec --report-bindings -np ${PROCESSES} --map-by socket:pe=${THREADS} --bind-to core ${MAIN} -${MODE} -r ${INPUT} ${OUTPUT}  #1> ${OUTPUTFOLDER}/deconding_result_${THREADS}_threads  2> ${OUTPUTFOLDER}/deconding_result_${THREADS}_threads_err


# How to run
# write SCRIPT='/home/michele.yin/HuffmanCODEC/clusterscripts/parallelDecoderFolder.sh'
# write INPUTFOLDER=path to folders to decode
