            ...
            with open (file, "r") as myfile:
                for line in myfile:
                    if 'CPU' in line and 'frequencies' in line:
                        ...
                        thread_data[thread][processor]['Frequencies']['CPU'] += float(line.split(' ')[-1])
                    elif 'Wall' in line and 'frequencies' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Frequencies']['Wall'] += float(line.split(' ')[-1])
                    elif 'CPU' in line and 'tree' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Tree']['CPU'] += float(line.split(' ')[-1])
                    elif 'Wall' in line and 'tree' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Tree']['Wall'] += float(line.split(' ')[-1])
                    elif 'CPU' in line and 'encode' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Encode']['CPU'] += float(line.split(' ')[-1])
                    elif 'Wall' in line and 'encode' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Encode']['Wall'] += float(line.split(' ')[-1])
                    elif 'CPU' in line and 'flush' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Flush']['CPU'] += float(line.split(' ')[-1])
                    elif 'Wall' in line and 'flush' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Flush']['Wall'] += float(line.split(' ')[-1])
                    elif 'CPU' in line and 'Overall' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Overall']['CPU'] += float(line.split(' ')[-1])
                    elif 'Wall' in line and 'Overall' in line:
                        processor = line.split(' ')[0]
                        thread_data[thread][processor]['Overall']['Wall'] += float(line.split(' ')[-1])
